...
function listHtmlRouteFiles() {
  ...
  function walk(dir) {
    for (const entry of readdirSync(dir)) {
      ...
      // Windows, and every assertion below compares against src/routes/...
...
test("__root.tsx CSP wraps every HTML route by construction", () => {
  ...
  // head() takes an options argument in the current router API; the
  // invariant under test is that __root defines head, not its arity.
